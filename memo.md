...
|---|---|---|---|
| 24/03/2023 | **Public editor xây cho AI, không chỉ extension.** Decision sở hữu editor cho phép Cursor thay đổi navigation, edit, terminal/LSP và feedback loop ở cấp sản phẩm. [HN launch](https://news.ycombinator.com/item?id=35285047) · [Founder interview](https://a16z.com/podcast/michael-truell-how-cursor-builds-at-the-speed-of-ai/) | HN launch cho thấy user sớm so sánh trực tiếp với editor hiện hữu + Copilot; founder sau đó xác nhận quyết định chiến lược là sở hữu editor. | **AI-native UX:** khi AI đổi primitive của công việc, product cần kiểm soát interaction layer đủ sâu. |
| 19/07/2023 | **Đưa context building vào codebase-wide chat**, đồng thời nối Cmd+K với linter feedback. [Nguồn](https://cursor.com/changelog/0-2-49) | Chat v2 trước đó dùng `@` để user chỉ file/code/docs; mốc này tăng quyền kiểm soát context ở cấp codebase. **INFERENCE:** bottleneck đã dịch từ “có model” sang “model thấy đúng phần repo và biết output có lỗi hay không”. | **Context > Model:** grounding và feedback đúng có thể tạo nhiều giá trị hơn chỉ nâng raw model. |
| 24/11/2024 | **Đưa Agent vào Composer:** tự chọn context, dùng terminal và trả inline diff. [Nguồn](https://cursor.com/changelog/0-43-x) | Repo context đã cho AI xử lý task lớn hơn completion; phần việc còn lại là user điều phối tìm file → sửa → chạy lệnh → đọc lỗi. Terminal access khép loop này. | **Assistant → Agent:** tăng delegation khi AI có context, tools, observable state và review path. |
...
**Agent trong Composer — 24/11/2024.** Đây là điểm Cursor chuyển từ “AI giúp sửa code” sang “AI nhận task và tự đóng loop bằng context + terminal”, với inline diff để user review.
